...
export default function AdminValidateTicketsPage() {
    ...
    <div className="min-h-screen pb-8">
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 px-6 pt-8 pb-12 rounded-b-[2rem]">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => navigate('/admin')} className="flex items-center gap-2 text-white/80 mb-4 hover:text-white transition-colors">
            <ArrowLeft className="w-5 h-5" /> Voltar ao Painel
          </button>
          <h1 className="font-display font-bold text-2xl text-white flex items-center gap-2">
            <Shield className="w-6 h-6 text-amber-400" />
            <ScanLine className="w-6 h-6" /> Validar Ingressos (ADM)
          </h1>
          <p className="text-white/60 text-sm mt-1">Valide ingressos de qualquer produtor</p>
        </div>
      </div>
              ...
              {result.valid ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/30">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Produtor: {result.producerName}</p>
                    </div>
                  </div>
                  {!validated && (
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30">
                  ...
                  <div>
                    ...
                    {result.event && <p className="text-sm text-muted-foreground mt-1">Evento: {result.event}</p>}
                    {result.producerName && <p className="text-sm text-muted-foreground">Produtor: {result.producerName}</p>}
                  </div>
                </div>
              ) : (
