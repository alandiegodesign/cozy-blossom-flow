...
import { Ticket, User, Briefcase, Eye, EyeOff, Lock, Mail, CreditCard } from 'lucide-react';
        ...
        <div className="w-full bg-card rounded-2xl border border-border p-1.5 flex mb-8">
          <button type="button" onClick={() => setUserType('cliente')}
            className={`flex-1 flex flex-col items-center gap-1 py-4 rounded-xl transition-all font-display font-semibold text-sm ${
              userType === 'cliente' ? 'bg-[hsl(145,63%,42%)] text-white shadow-lg' : 'text-muted-foreground hover:text-foreground'
            }`}>
          ...
          <button type="button" onClick={() => setUserType('produtor')}
            className={`flex-1 flex flex-col items-center gap-1 py-4 rounded-xl transition-all font-display font-semibold text-sm ${
              userType === 'produtor' ? 'bg-[hsl(145,63%,42%)] text-white shadow-lg' : 'text-muted-foreground hover:text-foreground'
            }`}>
          ...
          <div className="flex gap-2 mb-2">
            <button type="button" onClick={() => { setLoginMethod('email'); setCredential(''); }}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${
                loginMethod === 'email' ? 'bg-muted text-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}>
              <Mail className="w-4 h-4 inline mr-1" /> Email
            </button>
            <button type="button" onClick={() => { setLoginMethod('cpf'); setCredential(''); }}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${
                loginMethod === 'cpf' ? 'bg-muted text-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}>
